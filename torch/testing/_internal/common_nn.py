...
from torch.testing._internal.common_utils import TestCase, to_gpu, freeze_rng_state, is_iterable, \
    TEST_WITH_ROCM, gradcheck, gradgradcheck, set_default_dtype
...
non_linear_activations_extra_info: Dict[str, dict] = {
    ...
    'Hardswish': {'default_dtype': torch.double},
...
classification_criterion_no_batch = [
    ('BCELoss', lambda: torch.sigmoid(torch.randn(9, dtype=torch.double)), lambda: torch.randn(9, dtype=torch.double).gt(0).to(torch.double)),
    ...
    ('CosineEmbeddingLoss', lambda: (torch.randn(9, dtype=torch.double), torch.randn(9, dtype=torch.double)), lambda: torch.tensor(1, dtype=torch.double)),
    ...
    ('TripletMarginLoss', lambda: (torch.randn(9, dtype=torch.double), torch.randn(9, dtype=torch.double)), lambda: torch.randn(9, dtype=torch.double)),
