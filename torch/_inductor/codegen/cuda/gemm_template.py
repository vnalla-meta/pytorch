...
{{template.define_gemm_instance(op)}}

{{kernel.def_kernel(inputs=[X, W, Bias], outputs=[Y], names_str="X, W, Bias, Y, uint8_t* workspace, size_t* workspace_size, cudaStream_t stream")}} {
  ...
  {{op.procedural_name()}} gemm_op;

  if (!workspace) {
    if (!workspace_size) {
        throw std::runtime_error("Both workspace and workspace_size are null!");
    }
...
class CutlassGemmTemplate(CutlassTemplate):
    ...
    def filter_op(
        ...
        def set_output_alignment(torch_layout, op_element, has_tma_epilogue, name) -> bool:
            ...
            op_element.layout = cutlass_layout
        ...
        if not set_output_alignment(self.output_node.layout, op.D, has_tma_epilogue, "output"):
            # print("skip output alignments")
            return None
        ...
        if len(self.input_nodes) == 3:
            ...
            if not set_output_alignment(Bias.get_layout(), op.C, has_tma_epilogue, "bias"):
